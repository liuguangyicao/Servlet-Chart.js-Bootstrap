...
public class Check {
	...
	public String checkLogin(Register student)  {
		String sql = "SELECT id,username FROM mdl_user WHERE username = '" + student.getUsername() + "';";
		DBconn dbconn = new DBconn();
		ResultSet rst = null;
		String id = null;
		...
		try {
			if(rst.next()) {
				checklogin =  rst.getString("username").equals(student.getPassword());
				id = rst.getString("id");
			} else {
				checklogin =  false;
			}
